...
const CHIPS = [
  { icon: '📚', label: '60,000+ Print Volumes' },
  { icon: '💻', label: 'Digital Library 24×7' },
  { icon: '📰', label: '3,338+ E-Journals' },
  { icon: '🎥', label: 'NPTEL Video Lectures' },
  { icon: '🏛',  label: '448 m² Reference Hall' },
  { icon: '🔗', label: 'FTP Intranet Access' },
  { icon: '🔍', label: 'OPAC Search System' },
  { icon: '📖', label: 'Research Journals' },
];
    ...
    <section className="bg-[#f7f8fc] py-24 max-[960px]:py-16 max-[600px]:py-12 px-[7%] max-[960px]:px-[5%] max-[600px]:px-[4%]">
      <div className="grid grid-cols-2 max-[960px]:grid-cols-1 gap-[72px] max-[960px]:gap-9 items-center">
        ...
        <div className="relative rounded-[20px] overflow-visible">
          ...
          <img
            src="/infrastructure/library/2image.jpg"
            alt="Tulas Library"
            className="w-full aspect-[4/3] object-cover rounded-[18px] block shadow-[0_24px_64px_rgba(0,0,0,0.12)]"
          />

          {/* Badge — desktop: absolute bottom-right; mobile: static below image */}
          <div className="absolute -bottom-5 -right-5 max-[960px]:right-0 max-[960px]:-bottom-4 max-[600px]:static max-[600px]:flex max-[600px]:items-center max-[600px]:gap-[14px] max-[600px]:text-left max-[600px]:rounded-[12px] max-[600px]:mt-4 max-[600px]:w-fit z-10 bg-[#111111] text-white rounded-[14px] px-[22px] py-[18px] max-[600px]:px-[18px] max-[600px]:py-[14px] font-montserrat font-extrabold shadow-[0_12px_32px_rgba(0,0,0,0.22)] min-w-[140px] max-[600px]:min-w-0 text-center">
            <span className="text-[1.9rem] max-[600px]:text-[1.7rem] leading-none text-[#f26522] block">60K+</span>
            <span className="text-[0.68rem] max-[600px]:text-[0.67rem] text-white/55 tracking-[0.07em] uppercase mt-1 max-[600px]:mt-0 block">Volumes in<br />the Collection</span>
          </div>
        </div>
        ...
        <div>
          ...
          >
            The Heart of<br />
            <em className="text-[#f26522] not-italic">Tula&apos;s Institute</em>
          </h2>

          <p className="text-[0.97rem] leading-[1.78] text-[#6b7280]">
            With over 60,000 volumes, Tulas Library is a veritable powerhouse of knowledge. It provides a vast and constantly updated resource base to ensure the overall development of students, faculty, researchers, and staff. We believe the library is not just a room of books — it is the intellectual core of the institution.
          </p>
          <p className="text-[0.97rem] leading-[1.78] text-[#6b7280] mt-[14px]">
            Ranked consistently among the Top 10 Engineering Colleges in Dehradun by independent surveys, Tulas Infrastructure — including the library — has been a decisive reason students from across India choose this institution.
          </p>
