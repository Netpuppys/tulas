...
<section className="relative min-h-screen md:min-h-[90vh] flex flex-col items-stretch overflow-hidden pt-[70px] max-[600px]:pt-[60px]">
      ...
      <div
        ...
        style={{ backgroundImage: "url('/new-campus/bannermain.jpg')" }}
      />

      {/* Gradient overlay */}
       <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.90) 10%, rgba(0,0,0,0.55) 50%, rgba(0,0,0,0.18) 100%)' }}
        />
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-end px-[7%] max-[600px]:px-[4%]">
        ...
        {/* Breadcrumb */}
        {/* <div className="flex items-center gap-2 text-[0.78rem] max-[600px]:text-[0.7rem] text-white/60 mb-[22px] max-[600px]:mb-4 font-medium tracking-[0.05em] uppercase">
        ...
        <div className="inline-flex items-center gap-2 bg-[#f26522] text-white rounded-[6px] px-[14px] py-[5px] max-[600px]:py-[4px] max-[600px]:px-[12px] font-montserrat font-bold text-[0.72rem] max-[600px]:text-[0.67rem] tracking-[0.12em] uppercase mb-5 max-[600px]:mb-[14px] w-fit">
          📚 Campus Life At Tulas
        </div>

        {/* Title */}
        <h1
          className="font-montserrat font-extrabold text-white leading-[1.05] mb-5 max-[600px]:mb-[14px] max-[600px]:text-[2rem]"
          style={{ fontSize: 'clamp(2.8rem, 5.5vw, 5rem)' }}
        >
          More Than A Campus<br />
          <em className="text-[#f26522] not-italic">A Whole Experience</em>
        </h1>

        {/* Description */}
        <p className="text-[1.05rem] max-[600px]:text-[0.9rem] leading-[1.75] text-white/80 max-w-[600px] mb-9 max-[600px]:mb-6">
          At Tula's, learning begins in classrooms and grows through experiences, friendships and opportunities.
        </p>
      ...
      <div className="relative z-10 grid grid-cols-4 max-[600px]:grid-cols-2 bg-[#1e1e1e]">
        {STATS.map((s, i) => (
          <div
            key={i}
            className="py-[22px] max-[600px]:py-[14px] max-[600px]:px-2 text-center border-r border-white/[0.08] last:border-r-0 max-[600px]:[&:nth-child(2)]:border-r-0 max-[600px]:[&:nth-child(3)]:border-t max-[600px]:[&:nth-child(4)]:border-t max-[600px]:[&:nth-child(3)]:border-t-white/[0.08] max-[600px]:[&:nth-child(4)]:border-t-white/[0.08]"
          >
            <span className="font-montserrat font-extrabold text-[#f26522] text-[1.8rem] max-[600px]:text-[1.3rem] max-[380px]:text-[1.15rem] block leading-none">{s.val}</span>
            <span className="text-[0.73rem] max-[600px]:text-[0.6rem] text-white/45 tracking-[0.08em] uppercase mt-[5px] block">{s.lbl}</span>
          </div>
        ))}
      </div>
    </section>
