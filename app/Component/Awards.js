    ...
    return (
        ...
        >
            <div className='w-full h-full relative'>
                ...
                <div 
                    style={isMobile? { left: "50%", transform: "translate(-50%, 0)"} : {}}
                    className={`absolute text-nowrap top-12 md:top-1/2 md:-translate-y-1/2 md:px-5 ${flipped? "md:right-full" : "md:left-full"}`}
                >
...
const Card = ({ image, date, title, content }) => {
    return (
        <div className='w-full md:max-w-[45vw] px-6 md:px-10'>
            ...
            <p className='text-xs ms:text-sm text-white opacity-80'>
                {date}
            </p>
  ...
  return (
    <div ref={ref} className='w-full md:h-screen flex md:flex-row flex-col relative md:-mb-[6vh]'>
        <div className='w-full md:w-1/2 h-80 via-zinc-50 md:h-full flex items-center justify-start md:justify-center bg-[#230512] md:py-20 relative'>
            ...
            <div ref={pointsRef} className='px-10 md:hidden overflow-x-scroll overflow-y-visible max-w-full w-full h-40 flex items-center justify-start'>
        ...
        <div className={`${inView? "md:animate-swipeRight relative w-full md:w-1/2" : "absolute w-full top-0 right-0 md:animate-swipeLeft"} h-[50rem] md:h-full overflow-hidden`}>
            <Image
                src={background}
            ...
            <div ref={scrollRef} className='w-full max-w-full flex items-center justify-start flex-col max-h-full h-full overflow-y-hidden bg-black bg-opacity-40 md:px-10 relative z-[2]'>
                <div className='w-full md:max-w-[50vw] h-fit'>
                    {awards.map((item, id) => (
                    <div key={id} ref={item.ref} className='min-w-[100vw] md:min-w-0 md:w-full h-[50rem] md:h-screen flex items-start md:items-center justify-center py-16 md:py-0'>
                        <Card
                            image={item.image}
                            date={item.date}
                            title={item.title}
