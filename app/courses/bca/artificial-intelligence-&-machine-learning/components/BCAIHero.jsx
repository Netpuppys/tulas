...
const STATS = [
  { num: '3-Year',      label: 'Programme' },
  { num: '122',         label: 'Credits · 6 Semesters' },
  { num: '9',           label: 'Industry Certifications' },
  { num: '2+ Apps',     label: 'Live Portfolio at Graduation' },
];

export default function BCAIHero() {
  return (
    <>
      <style dangerouslySetInnerHTML={{__html:`
        @keyframes cl-bcaih-zoom{from{transform:scale(1)}to{transform:scale(1.08)}}
        .cl-bcaih-bg img{width:100%;height:100%;object-fit:cover;animation:cl-bcaih-zoom 20s ease-in-out infinite alternate}
        .cl-bcaih-strip{display:grid;grid-template-columns:repeat(4,1fr);gap:0;max-width:920px;margin:0 auto;background:rgba(255,255,255,.08);backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.18);border-radius:18px;overflow:hidden}
        @media(max-width:968px){.cl-bcaih-strip{grid-template-columns:repeat(2,1fr)}.cl-bcaih-si:nth-child(2){border-right:none}}
        @media(max-width:640px){.cl-bcaih-strip{grid-template-columns:1fr}.cl-bcaih-si{border-right:none!important;border-bottom:1px solid rgba(255,255,255,.14)}}
      `}} />
      ...
      >
        <div className="cl-bcaih-bg absolute inset-0 z-0">
          <img src="/courses/bca/departmentalMission.webp" alt="BCA AI & ML at Tulas Institute Dehradun" />
        </div>
        ...
        <div className="relative z-[5] text-center py-[50px] px-[7%] max-w-[1200px] w-full">
          ...
          >
            Home / Programs / BCA / <span style={{ color: '#FFF3D7' }}>Artificial Intelligence &amp; Machine Learning</span>
          </div>
          ...
          >
            Every student learns the full developer stack — Python, JavaScript, React, databases, cloud, and AI tools — and ships a deployed application every year. By graduation you leave with at least two live, working apps any employer or client can open and test.
          </p>
          ...
          <div className="cl-bcaih-strip">
            {STATS.map((item, i) => (
              <div
                key={i}
                className="cl-bcaih-si py-[24px] px-[18px] text-center"
