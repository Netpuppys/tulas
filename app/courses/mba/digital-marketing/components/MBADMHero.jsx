...
const STATS = [
  { num: '₹22 LPA',        label: 'Highest Indicative Range' },
  { num: 'AI-Integrated',  label: '3-Semester AI Curriculum' },
  { num: '3 Paths',        label: 'Career Tracks at Graduation' },
  { num: '9 Certs',        label: 'Industry Certifications' },
];

export default function MBADMHero() {
  return (
    <>
      <style dangerouslySetInnerHTML={{__html:`
        @keyframes cl-dmh-zoom{from{transform:scale(1)}to{transform:scale(1.08)}}
        .cl-dmh-bg img{width:100%;height:100%;object-fit:cover;animation:cl-dmh-zoom 20s ease-in-out infinite alternate}
        .cl-dmh-strip{display:grid;grid-template-columns:repeat(4,1fr);gap:0;max-width:920px;margin:0 auto;background:rgba(255,255,255,.08);backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.18);border-radius:18px;overflow:hidden}
        @media(max-width:968px){.cl-dmh-strip{grid-template-columns:repeat(2,1fr)}.cl-dmh-si:nth-child(2){border-right:none}}
        @media(max-width:640px){.cl-dmh-strip{grid-template-columns:1fr}.cl-dmh-si{border-right:none!important;border-bottom:1px solid rgba(255,255,255,.14)}}
      `}} />
      ...
      >
        {/* Background image */}
        <div className="cl-dmh-bg absolute inset-0 z-0">
          <img
            src="/courses/btech/computer-science-engineering-artifical-intelligence-and-machine-learning/departmentalVision.jpg"
            alt="MBA Digital Marketing at Tulas Institute"
          />
        </div>
        ...
        <div className="relative z-[5] text-center py-[50px] px-[7%] max-w-[1200px] w-full">
          ...
          >
            Home / Programs / MBA / <span style={{ color: '#FFF3D7' }}>Digital Marketing using AI</span>
          </div>
          ...
          >
            MBA · Digital Marketing using AI · NEP 2020
          </div>
          ...
          >
            Master the management core and applied digital marketing at Tulas Institute, Dehradun — and graduate job-ready with a live brand campaign, a CMO-panel capstone, and three career paths waiting at the finish line.
          </p>
          ...
          {/* Stats strip */}
          <div className="cl-dmh-strip">
            {STATS.map((item, i) => (
              <div
                key={i}
                className="cl-dmh-si py-[24px] px-[18px] text-center"
