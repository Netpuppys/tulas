...
const PILLS = [
  { bold: '3-Year',        rest: ' BBA Program'           },
  { bold: 'AI-Integrated', rest: ' Curriculum'            },
  { bold: null,            rest: 'Live Business Projects'  },
  { bold: null,            rest: 'Industry Certifications' },
  { bold: 'Up to ₹60 LPA',rest: ' Highest Package'       },
];

export default function BBABAHero() {
  return (
    <>
      <style dangerouslySetInnerHTML={{__html:`
        @keyframes cl-ba-zoom{from{transform:scale(1)}to{transform:scale(1.08)}}
        .cl-ba-bg img{width:100%;height:100%;object-fit:cover;animation:cl-ba-zoom 20s ease-in-out infinite alternate}
      `}} />
      ...
      >
        {/* Background image */}
        <div className="cl-ba-bg absolute inset-0 z-0">
          <img src="/courses/bba/business/mech.jpg" alt="BBA Business Analytics at Tulas" />
        </div>
        ...
        <div className="relative z-[5] text-center py-[50px] px-[7%] max-w-[1200px] w-full">
          ...
          >
            Home / Programs / BBA / <span style={{ color: '#FFF3D7' }}>Business Analytics</span>
          </div>
          ...
          >
            ...
            <em className="not-italic" style={{ color: '#FFF3D7', filter: 'drop-shadow(0 4px 16px rgba(255,243,215,.5))' }}>
          ...
          {/* Subtitle */}
          <p
            className="text-[1.14rem] leading-[1.9] max-w-[860px] mx-auto mb-[30px]"
            style={{ color: 'rgba(255,255,255,.92)' }}
          >
            Master business strategy through data with an AI-integrated BBA in Business Analytics. Learn data visualization, business intelligence, predictive analytics, AI tools, financial analytics, and decision-making while solving real business problems using live datasets.
          </p>

          {/* Pills */}
          <div className="flex flex-wrap gap-[10px] justify-center max-w-[900px] mx-auto mb-[34px]">
            {PILLS.map((pill, i) => (
              <span
                key={i}
