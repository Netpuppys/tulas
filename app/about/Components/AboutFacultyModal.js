...
const AboutFacultyModal = ({ setActiveProfile, activeProfile, relatedProfiles }) => {
            ...
            <div className="bg-[#3D001B] px-12 py-10">
                ...
                <div className="w-full flex flex-wrap items-center justify-center gap-6 mt-14">
                    {relatedProfiles?.map((card, groupIndex) => (
                        <div
                        key={groupIndex}
                        onClick={() => handleProfileChange({ profile: card, related: relatedProfiles })}
                        className={`flex-col max-w-[calc(33%-1.5rem)] items-center justify-center gap-5 ${activeProfile.name === card.name? "hidden" : "flex"}`}
                        >
                            <Fade>
                                <Image
                                src={card.image}
