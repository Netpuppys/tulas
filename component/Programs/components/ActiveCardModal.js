...
const ActiveCardModal = ({ card, setActiveCard, items, heading }) => {
    ...
    return (
        <div className='w-screen h-screen z-[999] top-0 left-0 fixed bg-black bg-opacity-40 backdrop-blur-sm flex items-start justify-end'>
            <div className='overflow-y-scroll z-[999] bg-white w-[45vw] h-full'>
                {/* top bar */}
                <div className="w-[45vw] h-20 bg-[#760135] flex items-center justify-between px-[1.3rem] py-4">
                    ...
                    <div className=''>
                        ...
                        <div className='mt-6'>
                            {card?.points?.map((item, index) => (
                                <p key={index} className=" text-[#1b1b1b] text-wrap text-lg font-light mt-6">
                                    {item}
                                </p>
                            ))}
                        </div>
                ...
                <div className='bg-[#3D001B] px-12 w-full py-10 text-center flex flex-col items-center justify-start'>
                    ...
                    {items.map((item, index) => (
                        <div
                            ...
                            className={`relative w-[calc(50%-2.5rem)] group transition-all duration-500 ease-in-out ${item.title === card.title? "hidden" : "block"} `}
                        >
                            <Image
                                src={item.image}
                                alt=""
